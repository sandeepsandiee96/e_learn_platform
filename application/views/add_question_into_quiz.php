 ...
 <h3><?php echo $title;?></h3>
  <a href="<?php echo site_url('exam/edit_exam/'.$quid);?>" class="btn btn-info"  ><?php echo $this->lang->line('close');?></a><br><br>
  <div class="row">
 
  <div class="col-lg-6">
    <form method="post" action="<?php echo site_url('exam/add_question/'.$quid.'/'.$limit.'/'.$cid.'/'.$lid);?>">
...
<br> 
		...
		<input type="hidden" id="added" value="<?php echo $this->lang->line('added');?>">
					...
					<form method="post" action="<?php echo site_url('exam/pre_add_question/'.$quid.'/'.$limit.'/'.$cid.'/'.$lid);?>">
					<select   name="cid">
...
<a href="<?php echo site_url('exam/add_question/'.$quid.'/'.$back.'/'.$cid.'/'.$lid);?>"  class="btn btn-primary"><?php echo $this->lang->line('back');?></a>
&nbsp;&nbsp;
<?php
 $next=$limit+($this->config->item('number_of_rows'));  ?>

<a href="<?php echo site_url('exam/add_question/'.$quid.'/'.$next.'/'.$cid.'/'.$lid);?>"  class="btn btn-primary"><?php echo $this->lang->line('next');?></a>
